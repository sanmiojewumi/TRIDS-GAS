    ...
    <div className="glass-card rounded-3xl p-6 sm:p-8 md:p-10 border border-[#1E3A8A] shadow-2xl relative">
      ...
      {success ? (
        <div className="p-8 bg-emerald-950/90 border border-emerald-500/40 rounded-2xl text-center space-y-4 animate-in zoom-in-95">
          ...
          <h4 className="text-2xl font-bold text-white font-heading">Booking Confirmed!</h4>
          <p className="text-slate-300 text-sm max-w-md mx-auto leading-relaxed">
            Your appointment for <strong className="text-amber-400">{formData.service}</strong> on{' '}
            <strong className="text-white">{formData.date} at {formData.time}</strong> has been logged. We will send a confirmation SMS/email to {formData.email} shortly.
          </p>
          <button
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          >
            {loading ? <span>Checking Schedule & Booking...</span> : <span>CONFIRM APPOINTMENT BOOKING</span>}
          </button>
        </form>
      )}
    </div>
