...
export async function sendEnquiryEmailNotification(params: SendEnquiryNotificationParams) {
    ...
    <body>
      <div class="card">
        ...
        <div class="row">
          ...
          <div class="value"><a href="tel:${params.phone}" style="color: #f59e0b; text-decoration: none;">${params.phone}</a></div>
        </div>

        <div class="row">
          <div class="label">Email Address</div>
          <div class="value"><a href="mailto:${params.email}" style="color: #38bdf8; text-decoration: none;">${params.email}</a></div>
        </div>

        <div class="row">
          <div class="label">Postcode / Location</div>
          <div class="value">${params.postcode}</div>
        </div>

        <div class="row">
          <div class="label">Requested Service</div>
          <div class="value" style="color: #f59e0b;">${params.service}</div>
        </div>

        ${params.preferredDate ? `
        <div class="row">
          <div class="label">Preferred Date & Time</div>
          <div class="value">${params.preferredDate} (${params.preferredTime || 'Flexible'})</div>
        </div>
        ` : ''}

        <div class="box">
          <div class="label" style="margin-bottom: 4px;">Job Description & Notes</div>
          <div className="value" style="color: #e2e8f0; font-weight: normal; font-size: 14px; line-height: 1.5;">${params.message}</div>
        </div>
  ...
  const mailOptions = {
    ...
    to: recipientEmail,
    subject: `🚨 NEW ENQUIRY: ${params.service} - ${params.name} (${params.postcode})`,
    html: htmlContent,
  };
...
export async function sendBookingEmailNotification(params: SendBookingNotificationParams) {
    ...
    <body>
      <div class="card">
        ...
        <div class="row">
          ...
          <div class="value" style="color: #10b981; font-size: 17px;">${params.date} at ${params.time}</div>
        </div>
        ...
        <div class="box">
          ...
          <div class="value" style="color: #e2e8f0; font-weight: normal; font-size: 14px; line-height: 1.5;">${params.notes}</div>
        </div>
        ` : ''}
  ...
  const mailOptions = {
    from: `"${settings.companyName} Web Portal" <${OFFICIAL_EMAIL}>`,
    to: recipientEmail,
    subject: `📅 BOOKING CONFIRMED: ${params.service} - ${params.date} ${params.time} (${params.customerName})`,
    html: htmlContent,
  };
